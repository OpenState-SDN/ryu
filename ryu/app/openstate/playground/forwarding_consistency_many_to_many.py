...
class OSLoadBalancing(app_manager.RyuApp):
    ...
    def add_flow_4(self, datapath, table_miss=False):
        ...
        if table_miss:
            ...
            for in_port in range(1,4):
                # if state=DEFAULT => send it to the first group entry in the group table
                actions = [
                        osparser.OFPExpActionSetState(state=in_port, table_id=1),
                        ofparser.OFPActionGroup(1)]
                match = ofparser.OFPMatch(
    ...
    def send_key_lookup_3(self, datapath):
        key_lookup_extractor = osparser.OFPExpMsgKeyExtract(datapath=datapath, command=osp.OFPSC_EXP_SET_L_EXTRACTOR, fields=[ofp.OXM_OF_IPV4_SRC,ofp.OXM_OF_IPV4_DST,ofp.OXM_OF_TCP_SRC,ofp.OXM_OF_TCP_DST],table_id=0)
        datapath.send_msg(key_lookup_extractor)
        key_lookup_extractor = osparser.OFPExpMsgKeyExtract(datapath=datapath, command=osp.OFPSC_EXP_SET_L_EXTRACTOR, fields=[ofp.OXM_OF_IPV4_SRC,ofp.OXM_OF_IPV4_DST,ofp.OXM_OF_TCP_SRC,ofp.OXM_OF_TCP_DST],table_id=1)
        datapath.send_msg(key_lookup_extractor)

    def send_key_update_3(self, datapath):
        key_update_extractor = osparser.OFPExpMsgKeyExtract(datapath=datapath, command=osp.OFPSC_EXP_SET_U_EXTRACTOR, fields=[ofp.OXM_OF_IPV4_SRC,ofp.OXM_OF_IPV4_DST,ofp.OXM_OF_TCP_SRC,ofp.OXM_OF_TCP_DST],table_id=0)
        datapath.send_msg(key_update_extractor)
        key_update_extractor = osparser.OFPExpMsgKeyExtract(datapath=datapath, command=osp.OFPSC_EXP_SET_U_EXTRACTOR, fields=[ofp.OXM_OF_IPV4_DST,ofp.OXM_OF_IPV4_SRC,ofp.OXM_OF_TCP_DST,ofp.OXM_OF_TCP_SRC],table_id=1)
        datapath.send_msg(key_update_extractor)
