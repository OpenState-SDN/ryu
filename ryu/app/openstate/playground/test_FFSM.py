...
class OSTestFFSM(app_manager.RyuApp):
    ...
    def state_stats_reply_handler(self, ev):
        ...
        if ev.msg.body.exp_type==0:
            ...
            stats = parser.OFPStateStats.parser(ev.msg.body.data, offset=0)
            for stat in stats:
                extractor = [ofp.OXM_OF_IPV4_SRC]
                print('{table_id=%s, key={%s}, state=%d, dur_sec=%d, dur_nsec=%d, idle_to=%d, idle_rb=%d, hard_to=%d, hard_rb=%d}' 
                        %(stat.table_id,parser.state_entry_key_to_str(extractor,stat.entry.key,stat.entry.key_count),
            ...
            stat = parser.OFPGlobalStateStats.parser(ev.msg.body.data, offset=0)
    ...
    def send_state_stats_request(self, datapath):
        ...
        req = ofp_parser.OFPExpStateStatsMultipartRequest(datapath=datapath)
        datapath.send_msg(req)
        ...
        #req = ofp_parser.OFPExpStateStatsMultipartRequest(datapath=datapath, table_id=ofproto.OFPTT_ALL, state=768)
