...
class OSLoadBalancing(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, table_miss=False):
        ...
        if table_miss:
            ...
            # Forward path flow
            # state=DEFAULT => send it to the first group entry in the group table
            actions = [
                    parser.OFPActionGroup(1)]
            match = parser.OFPMatch(
                    in_port=1, state=0, eth_type=0x800, ip_proto=6)
            inst = [
    ...
    def send_group_mod(self, datapath):
            ...
            buckets = []
            # Action Bucket: <FWD port_i , SetState(i)
            for port in range(2,SWITCH_PORTS+1):
                ...
                actions = [
                    ...
                    ofp_parser.OFPActionSetState(state=port, table_id=0),
                    ]
    ...
    def send_key_lookup(self, datapath):
        ofp = datapath.ofproto
        key_lookup_extractor = datapath.ofproto_parser.OFPKeyExtract(datapath, command=ofp.OFPSC_SET_L_EXTRACTOR,
    ...
    def send_key_update(self, datapath):
        ofp = datapath.ofproto
        key_update_extractor = datapath.ofproto_parser.OFPKeyExtract(datapath, command=ofp.OFPSC_SET_U_EXTRACTOR,
