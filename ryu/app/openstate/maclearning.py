...
#port considere in_port and state=metadata: matching headers are in_port + metadata
    ...
    def add_flow(self, datapath,table_miss=False):
	    ...
	    datapath.send_msg(mod)
            ...
            for in_port in range (1,port_num+1):
                print "here is flow mod install rules for port  %d\n" % in_port
                for state in range(port_num+1):
	            if state==0:
	                actions = [datapath.ofproto_parser.OFPActionOutput(ofproto.OFPP_FLOOD)]
                        match = datapath.ofproto_parser.OFPMatch(in_port=in_port,metadata=state)
	            else:
		        actions = [datapath.ofproto_parser.OFPActionOutput(state,0)]
                        match = datapath.ofproto_parser.OFPMatch(in_port=in_port,metadata=state)
	                
                    inst = [datapath.ofproto_parser.OFPInstructionState(in_port),datapath.ofproto_parser.OFPInstructionActions(datapath.ofproto.OFPIT_APPLY_ACTIONS, actions)]
