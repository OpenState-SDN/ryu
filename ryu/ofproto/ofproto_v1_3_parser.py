...
def OFPExpMsgConfigureStatefulTable(datapath, statefulness, table_id):
    ...
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(ofproto.OFP_EXP_STATE_MOD_STATEFUL_TABLE_CONFIG_PACK_STR,table_id,statefulness)
    
    exp_type=ofproto.OFPT_EXP_STATE_MOD
...
def OFPExpMsgKeyExtract(datapath, command, fields, table_id):
    ...
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(ofproto.OFP_EXP_STATE_MOD_EXTRACTOR_PACK_STR,table_id,field_count)
...
def OFPExpMsgSetFlowState(datapath, state, keys, table_id, state_mask=0xffffffff):
    key_count=len(keys)
    command=ofproto.OFPSC_EXP_SET_FLOW_STATE
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(ofproto.OFP_EXP_STATE_MOD_SET_FLOW_STATE_PACK_STR,table_id,key_count,state,state_mask)
...
def OFPExpMsgDelFlowState(datapath, keys, table_id):
    key_count=len(keys)
    command=ofproto.OFPSC_EXP_DEL_FLOW_STATE
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(ofproto.OFP_EXP_STATE_MOD_DEL_FLOW_STATE_PACK_STR,table_id,key_count)
    field_extract_format='!B'
...
def OFPExpSetGlobalState(datapath, flag, flag_mask=0xffffffff):
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, ofproto.OFPSC_SET_GLOBAL_STATE)
    data+=struct.pack(ofproto.OFP_EXP_STATE_MOD_SET_GLOBAL_STATE_PACK_STR,flag,flag_mask)
    
    exp_type=ofproto.OFPT_EXP_STATE_MOD
    return OFPExperimenter(datapath=datapath, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpResetGlobalState(datapath):
    data=struct.pack(ofproto.OFP_EXP_STATE_MOD_PACK_STR, ofproto.OFPSC_RESET_GLOBAL_STATE)
...
def OFPExpStateStatsMultipartRequest(datapath, flags=0, table_id=ofproto.OFPTT_ALL, match=None):
    ...
    data=bytearray()
    msg_pack_into(ofproto.OFP_STATE_STATS_REQUEST_0_PACK_STR, data, 0, table_id)
    
    offset=ofproto.OFP_STATE_STATS_REQUEST_0_SIZE
    match.serialize(data, offset)

    exp_type=ofproto.OFPMP_EXP_STATE_STATS
    return OFPExperimenterStatsRequest(datapath=datapath, flags=flags, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpGlobalStateStatsMultipartRequest(datapath, flags=0):
    data=bytearray()

    exp_type=ofproto.OFPMP_EXP_FLAGS_STATS
    return OFPExperimenterStatsRequest(datapath=datapath, flags=flags, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)
