...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def test4(self,datapath):
        ...
        state = datapath.ofproto_parser.OFPExpMsgSetStateEntry(datapath, ofproto.OFPSC_EXP_SET_FLOW_STATE, 14, 88, [0,0,0,0,0,2,0,0,0,0,0,4,8],cookie=0, cookie_mask=0, table_id=0)
        datapath.send_msg(state)
        # I provide and declared zero keys => it's consistent but I cannot access the state table with an empty key!
        state = datapath.ofproto_parser.OFPExpMsgSetStateEntry(datapath, ofproto.OFPSC_EXP_SET_FLOW_STATE, 0, 88, [],cookie=0, cookie_mask=0, table_id=0)
        datapath.send_msg(state)
        # I provide and declared more keys than allowed
        state = datapath.ofproto_parser.OFPExpMsgSetStateEntry(datapath, ofproto.OFPSC_EXP_SET_FLOW_STATE, 49, 88, [0,0,0,0,0,2,0,0,0,0,0,4,0,0,0,0,0,2,0,0,0,0,0,4,0,0,0,0,0,2,0,0,0,0,0,4,0,0,0,0,0,2,0,0,0,0,0,4,5],cookie=0, cookie_mask=0, table_id=0)
        datapath.send_msg(state)
    ...
    def test6(self,datapath):
        ...
        key_update_extractor = datapath.ofproto_parser.OFPExpMsgKeyExtract(datapath, ofp.OFPSC_EXP_SET_U_EXTRACTOR, 1, [ofp.OXM_OF_ETH_SRC])
        datapath.send_msg(key_update_extractor)
        state = datapath.ofproto_parser.OFPExpMsgSetStateEntry(datapath, ofproto.OFPSC_DEL_FLOW_STATE, 4, 99, [10,0,0,5],cookie=0, cookie_mask=0, table_id=0)
        datapath.send_msg(state)
